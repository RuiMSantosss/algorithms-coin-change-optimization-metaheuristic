...
int evaluateIndividual(int solution[], struct info data, float *coinsTypeArray, int *valid) {
	int i, totalCoins = 0;
	float sum_value = 0;
	...
	if (fabs(sum_value - data.target) < 0.0001) { // Ajusta tolerância
		*valid = 1;  // Solução válida
		return totalCoins;  // Fitness: menor número de moedas
	}
